...
async def main():
    ...
    async with ClientSession() as session:
        ...
        atlas_max_update_id = await atlas.max_update_id()
        print(f"Max Update ID: {atlas_max_update_id}")
        await asyncio.sleep(1)

        atlas_max_ffn_story_id = await atlas.max_story_id()
        ...
        atlas_ffn_spec_metadata = await atlas.get_story_metadata(atlas.extract_fic_id(ffn_test_url_1))
        print(f"FFN Specific Metadata: {atlas_ffn_spec_metadata}")
        await asyncio.sleep(1)

        atlas_ffn_spec_metadata2 = await atlas.get_story_metadata(atlas.extract_fic_id(ffn_test_url_2))
